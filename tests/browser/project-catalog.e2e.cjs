...
async function main() {
  ...
  for (const [label, width, height] of [['desktop', 1440, 900], ['tablet', 768, 900], ['mobile', 375, 812]]) {
    ...
    const state = await evaluate(`(() => {
      ...
      return {
        index,total:projs.length,unique:new Set(projs.map(item=>item.title)).size,project,
        filtered:filteredTitles.includes('PageTurn'),heroCount,fullStackReadout,
        allReadout,miniReadout,miniTitles,minis,
    ...
    assert(state.total===9&&state.unique===9, `${label}: expected nine unique projects`);
    ...
    assert(state.heroCount==='09'&&/\/ 09$/.test(state.allReadout), `${label}: overall project totals were not updated to 09`);
    ...
    await evaluate("closeModal(); applyProjectFilter('all'); true");
    results[label] = state;
  }

  await evaluate(`(() => {
    const required=['PageTurn','CG Algorithm Visualizer','Algorithm Selection Expert System','AI Prompt Quality Analyzer'];
    const legacy=projs.filter(project=>!required.includes(project.title));
  ...
  assert(migration.pageTurn===1&&migration.cg===1&&migration.algorithm===1&&migration.prompt===1&&migration.custom===1&&migration.unique===10&&migration.total===10, 'project migration did not add required projects once while preserving custom projects');
